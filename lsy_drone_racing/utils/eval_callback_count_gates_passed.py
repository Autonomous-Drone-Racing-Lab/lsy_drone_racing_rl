...
class EvalCallbackCountGatesPassed(EventCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            std_no_gates_passed = np.std(no_gates_passed)
            ...
            if self.verbose >= 1:
                ...
                print(f"Mean no gates passed: {mean_no_gates_passed:.2f} +/- {std_no_gates_passed:.2f}")
            # Add to current Logger
            self.logger.record("eval/mean_reward", float(mean_reward))
            self.logger.record("eval/mean_ep_length", mean_ep_length)
            self.logger.record("eval/mean_no_gates_passed", mean_no_gates_passed)
