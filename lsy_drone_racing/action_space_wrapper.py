"""Action space wrappers to convert policy actions into rcontroller actions.
...
class ActionSpaceWrapper(ABC):
    ...
    def __init__(self, config: Munch):
        """Initialize the action space wrapper.

        Args:
            config (Munch): The configuration
        """
        self.config = config

    
    def scale_action(self, action: np.ndarray, drone_pose: np.ndarray):
...
def action_space_wrapper_factory(config: Munch) -> ActionSpaceWrapper:
...
class ActionSpaceWrapperXYZ(ActionSpaceWrapper):
    ...
    def scale_action(self, action: np.ndarray, drone_pose: np.ndarray) -> np.ndarray:
        """Scale the given action based on the drone's pose.
...
class ActionSpaceWrapperXYZYaw(ActionSpaceWrapper):
    """Wrapper converting normed [x,y,z] and yaw into global coordinates.

    Implementation like ActionSpaceWrapperXYZ. However, the action space includes a yaw action.
...
class ActionSpaceWrapperXYZRelative(ActionSpaceWrapper):
    """Action space wrapper converting relative xyz coordinates to global unnormalized coordinates.
...
class ActionSpaceWrapperXYZRelativeYaw(ActionSpaceWrapper):
    """Relative ActionSpaceWrapper like (ActionSpaceWrapperXYZRelative), however with yaw.
    
    In addition to "ActionSpaceWrapperXYZRelative" implementation, yaw is provided as normalized global [-1, 1] ) (no offset) and is scaled to the range [-pi, pi].
    """
    def __init__(self, config: Munch):
        """Initialize the action space wrapper.
