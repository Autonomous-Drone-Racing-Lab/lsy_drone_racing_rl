...
class Experiment_1_Environment_Transformation:
    ...
    @staticmethod
    def transform_observation(obs: np.ndarray):
        """
        ...
        : returns observation [drone_xyz_yaw, cornes of next gate in local frame of drone]
        """
        ...
        gate_corners = translate_points_in_local_frame(drone_xyz_yaw, gate_corners).flatten() * 0 #Todo, update this

        obs = np.concatenate([drone_xyz_yaw, gate_corners]).astype(np.float32)
        return obs
